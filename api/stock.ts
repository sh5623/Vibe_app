...
function getParam(req: VercelRequest, key: string, fallback: string): string {
  ...
  try {
    ...
    if (mappedSymbol !== undefined) {
      ...
      try {
        ...
        if (searchResult.quotes && searchResult.quotes.length > 0) {
          ...
          const topQuote =
            quotes.find((q) => q.quoteType === 'EQUITY' || q.quoteType === 'ETF') ?? quotes[0]
    ...
    if (symbol === '064400.KS') {
      try {
        ...
        let history: YFHistoryPoint[] = []
        try {
          const fallbackChart = await yahooFinance.chart(fallbackSymbol, {
            period1,
            period2,
            interval: '1d',
          })
          history = ((fallbackChart.quotes ?? []) as YFHistoryPoint[]).filter(
            (q) => q.close !== null
          )
        } catch (e) {
