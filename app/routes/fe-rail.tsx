...
const TEXT = {
  ko: {
    ...
    workflow: { label: 'Workflow', title: '원스톱 자동화', desc: '단 두 번의 확인으로 PR까지 완성됩니다.' },
    install: {
      label: 'Install',
      title: '시작하기',
  ...
  en: {
    ...
    workflow: { label: 'Workflow', title: 'One-Shot Automation', desc: 'From feature.md to PR with only two confirmations.' },
    install: {
      label: 'Install',
      title: 'Get Started',
...
const SKILLS: Record<Lang, Array<{ id: string; icon: string; name: string; title: string; desc: string; cmd: string }>> = {
  ko: [
    ...
    { id: 'fe-build', icon: '🔨', name: 'fe-build', title: '코드 구현', desc: '타입 정의 → 훅/서비스 → 컴포넌트 → 테스트 순서로 구현합니다. Biome 또는 ESLint·Prettier 자동 적용, tsc/린터 오류 자동 수정.', cmd: '/fe-rail:fe-build' },
  ...
  en: [
    ...
    { id: 'fe-build', icon: '🔨', name: 'fe-build', title: 'Code Implementation', desc: 'Implements in order: type definitions → hooks/services → components → tests. Applies Biome or ESLint·Prettier automatically and auto-fixes tsc/linter errors.', cmd: '/fe-rail:fe-build' },
...
const AGENTS_KO = {
  spec: [
    ...
    { name: 'fe-architect', role: 'Next.js(RSC 경계) / Vite SPA(TanStack Router·React Router 7) 아키텍처 자문', model: 'opus' },
  ],
  build: [
  ...
  review: [
    ...
    { name: 'fe-a11y-auditor', role: 'a11y 8축 감사 (Tailwind 팔레트 Color Contrast 포함)', model: 'sonnet' },
    { name: 'fe-perf-auditor', role: 'Next.js(RSC·next/image·next/font) / Vite SPA(번들·fetchpriority) 성능 감사', model: 'sonnet' },
...
const AGENTS_EN = {
  spec: [
    ...
    { name: 'fe-architect', role: 'Next.js(RSC boundary) / Vite SPA(TanStack Router · React Router 7) architecture advice', model: 'opus' },
  ],
  build: [
    ...
    { name: 'fe-build-fixer', role: 'Minimal diff fixes for tsc/linter (ESLint/Biome) errors', model: 'sonnet' },
  ],
  review: [
    { name: 'fe-reviewer', role: 'Integrated 4-axis review', model: 'opus' },
    { name: 'fe-a11y-auditor', role: 'a11y 8-axis audit (Tailwind palette Color Contrast included)', model: 'sonnet' },
    { name: 'fe-perf-auditor', role: 'Next.js(RSC · next/image · next/font) / Vite SPA(bundle · fetchpriority) performance audit', model: 'sonnet' },
    { name: 'fe-test-runner', role: 'Test execution + failure classification', model: 'sonnet' },
    { name: 'fe-refactor-advisor', role: '6-dimension refactoring analysis + Before/After', model: 'opus' },
  ],
  pr: [
...
export default function FeRailPage() {
  ...
  const sectionClass = 'px-12 py-32 border-t border-[rgba(99,102,241,0.08)] even:bg-white/[0.02] max-[768px]:px-6 max-[768px]:py-20';
  const innerClass = 'max-w-[1200px] mx-auto w-full';
  const labelClass = 'text-[0.72rem] text-[#818cf8] tracking-[0.3em] uppercase mb-3';
  const titleClass = 'text-[clamp(1.8rem,4vw,3.5rem)] font-extrabold tracking-[-0.025em] leading-[1.1] mb-4 text-[#f1f5f9]';
  const descClass = 'text-[0.95rem] text-[#64748b] leading-[1.8] max-w-[560px] mb-16 max-[768px]:mb-10';
  ...
  return (
    ...
    >
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-12 py-5 bg-[rgba(7,11,26,0.88)] backdrop-blur-[20px] border-b border-[rgba(99,102,241,0.12)] max-[768px]:px-6 max-[768px]:py-4">
        ...
        <div className="flex items-center gap-5">
          ...
          >
            ...
            />
            <span className="relative z-[1] py-[0.38rem] px-[0.9rem] transition-colors duration-[250ms]" style={{ color: lang === 'ko' ? '#ffffff' : '#475569' }}>한</span>
            <span className="relative z-[1] py-[0.38rem] px-[0.9rem] transition-colors duration-[250ms]" style={{ color: lang !== 'ko' ? '#ffffff' : '#475569' }}>EN</span>
          </button>
          <a
      ...
      <section className="min-h-screen flex flex-col justify-center items-center px-12 pt-32 pb-24 relative overflow-hidden text-center max-[768px]:px-6 max-[768px]:pt-28 max-[768px]:pb-20">
        <div className="absolute w-[700px] h-[700px] rounded-full pointer-events-none top-[-180px] right-[-120px] max-[768px]:w-[385px] max-[768px]:h-[385px]"
          style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.22) 0%, transparent 65%)', animation: 'orbFloat 9s ease-in-out infinite' }} />
        <div className="absolute w-[500px] h-[500px] rounded-full pointer-events-none bottom-[-80px] left-[-80px] max-[768px]:w-[275px] max-[768px]:h-[275px]"
          style={{ background: 'radial-gradient(circle, rgba(52,211,153,0.12) 0%, transparent 65%)', animation: 'orbFloat 13s ease-in-out infinite reverse' }} />

        <motion.div variants={stagger} initial="hidden" animate="visible" className="flex flex-col items-center z-[1]">
          <motion.div variants={fadeUp}>
            <AnimatePresence mode="wait">
              <motion.div key={`badge-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
                <div className="inline-flex items-center gap-[0.6rem] text-[0.72rem] text-[#818cf8] tracking-[0.18em] uppercase bg-[rgba(99,102,241,0.1)] border border-[rgba(99,102,241,0.22)] rounded-[100px] py-[0.45rem] pl-3 pr-[1.1rem] mb-8 backdrop-blur-[8px]"
                  style={{ fontFamily: 'var(--font-mono), monospace' }}>
          ...
          <motion.div variants={fadeUp}>
            <AnimatePresence mode="wait">
              <motion.div key={`title-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
                <h1 className="text-[clamp(2.4rem,7vw,6rem)] font-extrabold leading-[1.1] tracking-[-0.03em] mb-6 max-w-[900px] text-[#f1f5f9] relative z-[1] max-[768px]:text-[clamp(2rem,8vw,3rem)]">
                  {t.hero.titlePre}{' '}
                  <span style={{ background: 'linear-gradient(135deg, #818cf8 0%, #34d399 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                    {t.hero.titleAccent}
                  </span>
                  {t.hero.titlePost}
                  <br />
          ...
          <motion.div variants={fadeUp}>
            <AnimatePresence mode="wait">
              <motion.div key={`sub-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
                <p className="text-[clamp(0.88rem,1.4vw,1.05rem)] text-[#64748b] leading-[1.85] max-w-[620px] mb-10 relative z-[1]"
                  style={{ fontFamily: 'var(--font-mono), monospace' }}>
          ...
          <motion.div variants={fadeUp}>
            <div className="flex gap-4 justify-center flex-wrap relative z-[1]">
              <a
                ...
                className="inline-flex items-center gap-2 py-[0.9rem] px-[2.25rem] bg-gradient-to-br from-[#6366f1] to-[#4f46e5] text-white no-underline rounded-[10px] transition-all duration-[250ms] shadow-[0_4px_20px_rgba(99,102,241,0.4)] cursor-pointer hover:-translate-y-[3px] hover:shadow-[0_12px_32px_rgba(99,102,241,0.55)] max-[480px]:flex-1 max-[480px]:justify-center"
                style={{ fontFamily: 'var(--font-mono), monospace', fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' }}
              >
                {t.hero.cta1}
              </a>
              <a
          ...
          <AnimatePresence mode="wait">
            <motion.div key={`skills-header-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <div className={labelClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.skills.label}</div>
                <h2 className={titleClass}>{t.skills.title}</h2>
                <p className={descClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.skills.desc}</p>
              </motion.div>
            </motion.div>
          </AnimatePresence>
        ...
        <div className={innerClass}>
          <AnimatePresence mode="wait">
            <motion.div key={`detail-header-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <div className={labelClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.skills.detailLabel}</div>
                ...
                <motion.div key={`detail-${skill.id}-${lang}`} variants={fadeUp}>
                  <div id={skill.id} style={{ scrollMarginTop: '100px' }}>
                    <div className="relative bg-white/[0.03] border border-[rgba(99,102,241,0.12)] rounded-[16px] p-8 overflow-hidden transition-all duration-[250ms] before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:h-[2px] before:bg-gradient-to-r before:from-[#6366f1] before:to-[#34d399] before:scale-x-0 before:origin-left before:transition-transform before:duration-[350ms] before:rounded-tl-[16px] before:rounded-tr-[16px] hover:border-[rgba(99,102,241,0.28)] hover:shadow-[0_8px_32px_rgba(99,102,241,0.12)] hover:bg-white/5 hover:before:scale-x-100">
                      ...
                      <h3 className="text-[1.05rem] font-bold mb-[0.6rem] tracking-[-0.01em] text-[#f1f5f9]">{skill.name}</h3>
        ...
        <div className={innerClass}>
          <AnimatePresence mode="wait">
            <motion.div key={`agents-header-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <div className={labelClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.agents.label}</div>
                <h2 className={titleClass}>{t.agents.title}</h2>
                <p className={descClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.agents.desc}</p>
              </motion.div>
            </motion.div>
          </AnimatePresence>
            ...
            >
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(210px,1fr))] gap-3 mb-10">
                {agents[phase].map((agent) => (
                  ...
                  >
                    <div className="text-[0.8rem] text-[#818cf8] mb-[0.45rem] font-semibold" style={{ fontFamily: 'var(--font-mono), monospace' }}>{agent.name}</div>
                    <div className="text-[0.8rem] text-[#64748b] leading-[1.55]">{agent.role}</div>
                    <div className="text-[0.62rem] text-[rgba(52,211,153,0.8)] mt-3 tracking-[0.05em]" style={{ fontFamily: 'var(--font-mono), monospace' }}>model: {agent.model}</div>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <div className={innerClass}>
          <AnimatePresence mode="wait">
            <motion.div key={`workflow-header-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <div className={labelClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.workflow.label}</div>
                <h2 className={titleClass}>{t.workflow.title}</h2>
                <p className={descClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.workflow.desc}</p>
              </motion.div>
            </motion.div>
          </AnimatePresence>
          <AnimatePresence mode="wait">
                ...
                <div className="flex flex-col max-w-[520px]">
                  {workflow.map((step, i) => (
                    <motion.div key={i} variants={fadeUp}>
                      <div className={`flex gap-6 items-start relative ${i < workflow.length - 1 ? 'pb-9 before:content-[\'\'] before:absolute before:left-[19px] before:top-[42px] before:w-px before:h-[calc(100%-42px)] before:bg-gradient-to-b before:from-[rgba(99,102,241,0.3)] before:to-transparent' : ''}`}>
                        <div className="w-10 h-10 rounded-full bg-[rgba(99,102,241,0.1)] border-[1.5px] border-[rgba(99,102,241,0.25)] flex items-center justify-center text-[0.78rem] text-[#818cf8] font-bold shrink-0"
                          style={{ fontFamily: 'var(--font-mono), monospace' }}>
                          {String(i + 1).padStart(2, '0')}
                        </div>
                        <div className="pt-[0.45rem]">
                          <div className="text-[0.88rem] font-semibold text-[#f1f5f9] mb-[0.35rem]" style={{ fontFamily: 'var(--font-mono), monospace' }}>{step.label}</div>
                          <div className="text-[0.82rem] text-[#64748b] leading-[1.65]">{step.desc}</div>
                        </div>
                      </div>
                    </motion.div>
                  ))}
        ...
        <div className={innerClass}>
          <AnimatePresence mode="wait">
            <motion.div key={`install-header-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <div className={labelClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.install.label}</div>
                <h2 className={titleClass}>{t.install.title}</h2>
                <p className={descClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.install.desc}</p>
              </motion.div>
            </motion.div>
          </AnimatePresence>
          <motion.div
          ...
          >
            <pre className="bg-[#020617] border border-[rgba(99,102,241,0.15)] rounded-[14px] py-7 px-8 text-[0.82rem] leading-[1.9] text-[#e2e8f0] overflow-x-auto shadow-[0_4px_24px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(99,102,241,0.08)] max-[768px]:py-5 max-[768px]:px-5 max-[768px]:text-[0.75rem]"
              style={{ fontFamily: 'var(--font-mono), monospace' }}>
        ...
        <div className={innerClass}>
          <AnimatePresence mode="wait">
            <motion.div key={`faq-header-${lang}`} variants={langSwitch} initial="enter" animate="center" exit="exit">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                <div className={labelClass} style={{ fontFamily: 'var(--font-mono), monospace' }}>{t.faq.label}</div>
                  ...
                  {faqs.map((faq, i) => (
                    <motion.div key={i} variants={fadeUp}>
                      <div className="bg-white/[0.03] border border-[rgba(99,102,241,0.1)] rounded-[14px] py-7 px-8 mb-3 transition-all duration-200 hover:border-[rgba(99,102,241,0.22)] hover:shadow-[0_4px_16px_rgba(99,102,241,0.08)] hover:bg-white/5">
                        <div className="text-[0.98rem] font-bold text-[#f1f5f9] mb-[0.65rem] tracking-[-0.01em]">{faq.q}</div>
                        <div className="text-[0.88rem] text-[#64748b] leading-[1.8]" style={{ fontFamily: 'var(--font-mono), monospace' }}>{faq.a}</div>
                      </div>
                    </motion.div>
                  ))}
                </div>
      ...
      <footer className="px-12 py-8 flex justify-between items-center border-t border-[rgba(99,102,241,0.08)] text-[0.68rem] text-[#334155] tracking-[0.1em] bg-black/20 max-[768px]:px-6 max-[768px]:py-6 max-[768px]:flex-col max-[768px]:gap-1 max-[768px]:text-center"
        style={{ fontFamily: 'var(--font-mono), monospace' }}>
        <span>{t.footer}</span>
        <span>by 이승호</span>
      </footer>
